...
class LLMAssistantAggregator(LLMContextAggregator):
    ...
    async def push_aggregation(self, caller: Optional[str] = None):
        ...
        if not self._aggregation:
            logger.debug(
                f"{self} push_aggregation called - caller {caller} self._aggregation is empty"
            )
            return
        ...
        if aggregation:
            ...
            logger.debug(
                f"{self} push_aggregation called - caller {caller} self._aggregation = {aggregation}"
            )
    ...
    def _context_updated_task_finished(self, task: asyncio.Task):
        ...
        # Surface failures loudly, with traceback: a dead on_context_updated
        # task previously failed silently (exception never retrieved), which
        # hid engine set_node crashes such as the Gemini LLMSpecificMessage
        # TypeError.
        if task.cancelled():
            return
        exc = task.exception()
